...
def generate_spriteblocks(project):
    """Generates blocks for each sprite in a project.
    
    Args:
        project (dict): the Scratch project to process.
        surrounding (list): list of blocks allowed to added
    Returns:
        A dictionary with sprite names as keys and
        a list of scripts for the sprite as values
        that can later be turned into text.
    """
    ...
    for target in targets:
        ...
        for block_id, block in target["blocks"].items():
            ...
            if is_start:
                subscript = generate_script(block_id, target["blocks"])
...
def format_prompt(sprites):
    prompt = ""
    for i, (sprite, blocks) in enumerate(sprites.items()):
        if i == 0:
            header = f"Here is your current code for {sprite}:\\n"
        else:
            header = f"And here is your current code for {sprite}:\\n"
        prompt += format_string(header) + "\n"
        prompt += block_string(blocks)
    return prompt

def main():
    # Get Scratch project URL
    if len(sys.argv) > 2:
        sys.exit("Usage: python prompt.py SCRATCH_PROJECT_URL")
    elif len(sys.argv) == 2:
        url = sys.argv[1]
    else:
    ...
    # Generate blocks for each sprite
    blocks = generate_spriteblocks(data)
    # Format into prompt-friendly text
    text = format_prompt(blocks)
